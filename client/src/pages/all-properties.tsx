    ...
    <Box>
      ...
      {allProperties.length > 0 && (
        <Box display="flex" gap={2} mt={3} flexWrap="wrap">
          <CustomButton backgroundColor="#475be8" color="#fcfcfc" title="Previous" handleClick={() => setCurrent((prev) => prev - 1)} disabled={!(current > 1)} />
          ...
          <CustomButton backgroundColor="#475be8" color="#fcfcfc" title="Next" handleClick={() => setCurrent((prev) => prev + 1)} disabled={current === pageCount} />
          <Select variant="outlined" color="info" displayEmpty required inputProps={{ 'aria-label': 'Without label' }} defaultValue={10} onChange={(e) => setPageSize(e.target.value ? +e.target.value : 10)}>
